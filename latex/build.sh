...
dir_for()  { case "$1" in v0) echo V0_internal;; p1) echo P1_resource;; p2) echo P2_biology;; p3) echo P3_systems;; p4) echo P4_mitochondrial;; esac; }
name_for() { case "$1" in
  ...
  p4) echo Myco_mitogenome_draft;; esac; }
...
case "${1:-all}" in
  all) for k in v0 p1 p2 p3 p4; do build_one "$k"; done ;;
  *)   build_one "${1}" ;;
esac
